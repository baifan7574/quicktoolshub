// URL Encoder/Decoder Tool - 纯前端实现
(function() {
    'use strict';
    
    function initURLEncoder() {
        const container = document.getElementById('url-encoder-tool');
        if (!container) return;
        
        const inputTextarea = container.querySelector('#url-input');
        const outputTextarea = container.querySelector('#url-output');
        ...
        function encode() {
            try {
                const encoded = encodeURIComponent(inputTextarea.value);
                outputTextarea.value = encoded;
            } catch (e) {
                outputTextarea.value = 'Error encoding URL';
            }
        }
        
        function decode() {
            try {
                const decoded = decodeURIComponent(inputTextarea.value);
                outputTextarea.value = decoded;
            } catch (e) {
                outputTextarea.value = 'Error decoding URL. Please check if the input is valid URL-encoded text.';
            }
        }
