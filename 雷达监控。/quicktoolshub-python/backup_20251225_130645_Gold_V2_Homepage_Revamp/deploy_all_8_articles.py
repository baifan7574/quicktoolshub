"""
添加 Image Converter 文章到博客并部署（生成包含所有 8 篇文章的完整 blog.py）
"""
import sys
sys.path.append('.')
...
from generate_converter_blog import ImageConverterBlogGenerator
import paramiko
from scp import SCPClient
import time

def create_complete_blog_with_converter():
    print("=" * 80)
    print("生成包含所有 8 篇文章的完整 blog.py")
    print("=" * 80)
    
    # 1. 生成所有文章
    print("\n步骤 1: 生成所有文章...")
    
    # PDF Compressor 文章（3篇）
...
""",
    ...
    # Image Compressor 文章（3篇）
    img_gen = ImageCompressorBlogGenerator()
    img_articles = img_gen.generate_all_articles()
    
    # Image Resizer 文章（1篇）
    resizer_gen = ImageResizerBlogGenerator()
    resizer_article = resizer_gen.generate_article()
    
    # Image Converter 文章（1篇）
    converter_gen = ImageConverterBlogGenerator()
    converter_article = converter_gen.generate_article()
    
    all_articles = pdf_articles + img_articles + [resizer_article, converter_article]
    
    print(f"✅ 已生成 {len(all_articles)} 篇文章")
    print(f"  - PDF Compressor: {len(pdf_articles)} 篇")
    print(f"  - Image Compressor: {len(img_articles)} 篇")
    print(f"  - Image Resizer: 1 篇")
    print(f"  - Image Converter: 1 篇 ✨ 新增！")
...
'''
    ...
    try:
        ...
        print("\n【Image Converter - 1篇】✨ 新增！")
        print("  8. http://soeasyhub.com/blog/how-to-convert-image-format-online-free")
        
        print("\n📚 Image Converter 文章亮点：")
        print("  ✅ 2500+ 词完整教程")
        print("  ✅ 常见格式转换详解（PNG→JPG, JPG→PNG, WebP, HEIC→JPG）")
        print("  ✅ 格式对比表（JPG vs PNG vs WebP）")
        print("  ✅ 平台特定建议（社交媒体、网站、邮件）")
        print("  ✅ WebP 深度解析")
        print("  ✅ 最佳实践和常见错误")
        print("  ✅ 多个指向 Image Converter 工具的链接")
        print("  ✅ 相关文章链接")
...
if __name__ == "__main__":
    create_complete_blog_with_converter()
