...
def get_tools_grouped():
    """获取按分类分组的所有工具，用于主页展示"""
    all_tools = get_tools_list(limit=100) # Get all tools
    grouped = {}
    
    # Define Category Order (The "Magazine" Flow)
    # We want Writing & Image tools at the top as they are high traffic
    preferred_order = [
         "Writing Assistant",
         "Image Tools",
         "Text Tools", 
    ...
    for tool in all_tools:
        cat_name = tool.get('categories', {}).get('name', 'Uncategorized')
        if cat_name not in grouped:
            grouped[cat_name] = []
        grouped[cat_name].append(tool)
        
    # Formatting as a list of tuples for ordered iteration in Jinja: [(CatName, [Tools]), ...]
    ordered_result = []
    
    # 1. Add preferred categories first
    for cat in preferred_order:
        if cat in grouped and grouped[cat]:
            ordered_result.append((cat, grouped[cat]))
            del grouped[cat] # Remove so we don't add it again
            
    # 2. Add any remaining categories
    for cat, tools in grouped.items():
        if tools:
            ordered_result.append((cat, tools))
