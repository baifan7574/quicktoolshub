...
class SupabaseTable:
    ...
    def insert(self, data):
        """插入数据"""
        request_headers = self.headers.copy()
        # Prefer representation to get the inserted data back
        request_headers['Prefer'] = 'return=representation'
        
        response = requests.post(self.url, headers=request_headers, json=data)
        response.raise_for_status()
...
def get_supabase():
    ...
    if _supabase_client is None:
        if not Config.SUPABASE_URL or not Config.SUPABASE_KEY:
            print("Warning: Supabase credentials not found. Using local mock mode.")
            return None
