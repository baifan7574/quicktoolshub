...
def blog_article(slug):
    ...
    related = []
    # Try to find articles in same category first
    if article.get('category'):
        related = [a for a in ARTICLES if a.get('category') == article['category'] and a['slug'] != slug]

    # Fallback: If no related articles (or too few), fill with recent articles
    if len(related) < 3:
        # Get other articles excluding current one and already selected ones
        existing_slugs = [r['slug'] for r in related]
        others = [a for a in ARTICLES if a['slug'] != slug and a['slug'] not in existing_slugs]
