...
try:
    ...
    if row:
        ...
        new_nodes = [
            ...
            {
                "parameters": {
                    # 使用 PowerShell 将原始 JSON 对象管道传递给 Python，避免字符串转义地狱
                    "command": f'powershell -Command "$input | & \'{py_path}\' \'{sync_script}\'"'
                },
                "name": "Super Sync Driver",
                "type": "n8n-nodes-base.executeCommand",
                "typeVersion": 1,
                "position": [400, 300],
                "id": "node-super-sync-v12"
            }
        ]
        ...
        conn.commit()
        print("Database V12 (Unified Super Sync) patch applied.")
    conn.close()
