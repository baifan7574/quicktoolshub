...
# ================= Configuration =================
TOKEN_FILE = os.path.join("doc", "Token..txt")  # Corrected path
...
class MatrixLibrarian:
    ...
    def _load_config(self):
        ...
        if os.path.exists(TOKEN_FILE):
            ...
            alt_path = os.path.join("..", "doc", "Token..txt")
        ...
        print("Config loaded from local Token file (development mode).")
        return config

    def fetch_pending_tasks(self) -> List[dict]:
        print(f"Fetching batch of 200 tasks (Universal Logic)...")
        try:
    ...
    def download_and_upload(self, pdf_url, slug):
        try:
            print(f"   Downloading (Bypassing SSL): {pdf_url}")
