...
# ================= Matrix Reporter (The Lead Auditor) - REGENERATION MODE =================
# Mission: REGENERATE 130 Defective PDFs with DeepSeek V3 + xhtml2pdf.
...
<body>
    ...
    <div class="fingerprint">
        ...
        Generated by Matrix Reporter v3.0 (Regen) | SoEasyHub Compliance Engine
    </div>
...
class MatrixReporterRegen:
    def __init__(self):
        ...
        if self.config.get('deepseek_key'):
             print("[Lead Auditor Engaged] Production Engine: DeepSeek V3 (FORCED)")
             self.client = OpenAI(api_key=self.config['deepseek_key'], base_url="https://api.deepseek.com")
             self.model = "deepseek-chat"
        else:
            raise ValueError("CRITICAL: Missing DeepSeek API Key. Cannot proceed with V3 regeneration.")
    ...
    def _load_config(self):
        ...
        if not config['url'] or not config['key'] or not config['deepseek_key']:
            try:
                with open(TOKEN_FILE, 'r', encoding='utf-8') as f:
                    for line in f:
                        ...
                        # if "groqapi" in line: config['groq_key'] = line.split(":")[1].strip() # Not needed for DeepSeek force
    ...
    def fetch_defective_records(self, limit=130):
        """Fetch records that ALREADY have a PDF URL (defective) and are refined."""
        # Fetching records that are refined AND HAVE a pdf_url (The "Fix it" logic)
        print(f"[Fetcher] Targeting up to {limit} records with existing PDFs for regeneration...")
    ...
    def generate_audit_logic(self, record):
        ...
        # steps = "\n".join(data.get('steps', [])) # Unused in prompt but available
    ...
    def process_all(self, limit=130):
        records = self.fetch_defective_records(limit)
        if not records:
            print("No defective records found (or all fixed).")
            return

        print(f"[Batch Regeneration] Processing {len(records)} audits...")
        for r in records:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--limit", type=int, default=130, help="Limit number of records to regenerate")
    args = parser.parse_args()

    reporter = MatrixReporterRegen()
    reporter.process_all(limit=args.limit)
